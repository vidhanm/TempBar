...
- `Usage.swift` — CPU % and RAM used via Mach `host_statistics` (no subprocesses).
...
- App lives in `/Applications`. "Launch at Login" is a custom-drawn `Toggle` view (NSSwitch does not paint its accent tint inside menus on macOS 26; SMAppService.status is unreliable for ad-hoc signed apps). Login item state is read/written via System Events AppleScript, run through an `osascript` subprocess (in-process `NSAppleScript` left ~20 MB of fragmented heap; footprint is ~11 MB idle now). Only checked when the menu opens.
